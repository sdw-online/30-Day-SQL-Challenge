...
-- ============================================
-- EXERCISE TABLES: Emergency response coordination
-- ============================================
...
-- TABLE 6: dispatches (linking table between incidents and responder_units)
...
-- Validation 1: Row counts for all tables
...
-- Validation 2: Users with no playlist entries (should be 3)
SELECT u.user_id, u.display_name
FROM users u
LEFT JOIN playlist_tracks pt ON u.user_id = pt.user_id
WHERE pt.playlist_track_id IS NULL;

-- Validation 3: Songs not in any playlist (should be 3)
SELECT s.song_id, s.title
FROM songs s
LEFT JOIN playlist_tracks pt ON s.song_id = pt.song_id
WHERE pt.playlist_track_id IS NULL;

-- Validation 4: Incidents with no dispatch (should be 4)
SELECT incident_id, incident_type, status
FROM incidents
WHERE incident_id NOT IN (SELECT incident_id FROM dispatches);

-- Validation 5: orphan playlist entries (user_id has no matching users row) - should be 2
SELECT pt.playlist_track_id, pt.user_id, pt.playlist_name
FROM playlist_tracks pt
LEFT JOIN users u ON pt.user_id = u.user_id
WHERE u.user_id IS NULL
ORDER BY pt.playlist_track_id;

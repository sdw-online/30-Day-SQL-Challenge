...
Today uses several tables: two simple demo tables for learning concepts, plus the CloudMetrics exercise tables.

**Demo tables:** `recipes` (5 rows), `apartments` (5 rows), and `parcels` (10 rows) - used to demonstrate INSERT, UPDATE, DELETE, TRUNCATE, and DROP with minimal complexity.

**Exercise tables:** `products` (30 rows) and `support_tickets` (250 rows) - CloudMetrics subscription products and customer support tickets.

**Support tables:** `staging_daily_metrics` (7 rows), `staging_user_sessions` (6 rows), `legacy_ticket_categories` (5 rows), and `legacy_pricing_v1` (4 rows) - used for TRUNCATE and DROP exercises.

Run the SQL in [setup.sql](setup.sql) to create all tables.
...
-- ============================================
-- EXERCISE: CloudMetrics Products
-- ============================================
...
-- ============================================
-- EXERCISE: Support Tickets (250 rows)
-- ============================================
...
-- Staging table: daily website metrics
...
-- Legacy table: old ticket categories
...
-- Legacy table: old pricing model
...
-- Expected: Open 105, In Progress 60, Resolved 60, Cancelled 25
```
...
- **UPDATE with WHERE:** Changes values in existing rows - the WHERE clause targets specific rows; always SELECT first to preview
- **DELETE with WHERE:** Removes entire rows permanently - there is no undo, always preview with SELECT first
- **TRUNCATE:** Empties an entire table instantly by removing all rows without scanning them - resets the SERIAL sequence, faster than DELETE for full clears
- **DROP:** Removes the entire table structure including all rows, columns, indexes, and constraints - the table ceases to exist entirely
- **DELETE vs TRUNCATE vs DROP:** DELETE removes specific rows; TRUNCATE empties a table fast; DROP removes the entire table structure
